...
const schema = require('../schemas/schema.category');
const init = require('../init');
const logger = init.logger;

const { createMetadata, updateMetadata, generateId } = require('../lib/metadataHandler');

const modelName = init.modelNames.category;
...
router.post("/", async (req, res, next) => {
	try {
		...
		logger.trace(`Category: ${JSON.stringify(body)}`);
...
router.put("/:id", async (req, res, next) => {
	try {
		...
		if (!existingData) return res.status(404).json({ message: 'Category not found' });

		const metadata = updateMetadata(req, existingData._metadata);
		body._metadata = metadata;
		logger.trace(`Category updated: ${req.params.id} : ${JSON.stringify(body)}`);
...
router.delete("/:id", async (req, res, next) => {
	try {
		...
		logger.trace(`Category deleted: ${req.params.id} : ${JSON.stringify(existingData)}`);
